...
const urls = {
  ...
  "00190": ["https://forces.ca/fr/carriere/officier-physiotherapie/", "https://forces.ca/en/career/physiotherapy-officer/"],
  ...
  "00197": ["https://forces.ca/fr/carriere/Officier-des-operations-aeriennes/", "https://forces.ca/en/career/Air-Operations-Officer/"],
...
for (const [id, [urlFr, urlEn]] of Object.entries(urls)) {
    // We already added urlFr and urlEn previously, so replace them
    const regex = new RegExp(`("id":\\s*"${id}"[\\s\\S]*?"abbreviation":\\s*".*?",)\\s*"urlFr":\\s*".*?",\\s*"urlEn":\\s*".*?",`);
    if (regex.test(content)) {
      content = content.replace(regex, `$1\n    "urlFr": "${urlFr}",\n    "urlEn": "${urlEn}",`);
    } else {
      const regexFallback = new RegExp(`("id":\\s*"${id}",[\\s\\S]*?"abbreviation":\\s*".*?",)`);
      content = content.replace(regexFallback, `$1\n    "urlFr": "${urlFr}",\n    "urlEn": "${urlEn}",`);
    }
}

fs.writeFileSync('src/services/jobs-data.ts', content, 'utf8');
console.log('Final links updated!');
